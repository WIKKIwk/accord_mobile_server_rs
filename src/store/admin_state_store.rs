...
mod tests {
    ...
    use super::{AdminSupplierStateStore, LmdbAdminSupplierStateStore};
    ...
    async fn lmdb_admin_state_round_trips_full_state() {
        let dir = tempfile::tempdir().expect("tempdir");
        let store =
            LmdbAdminSupplierStateStore::open(dir.path().join("admin.lmdb"), 1024 * 1024, None)
                .expect("lmdb store");
    ...
    async fn lmdb_admin_state_migrates_legacy_json_for_all_ports() {
        ...
        let store = LmdbAdminSupplierStateStore::open(
            lmdb_path.clone(),
            1024 * 1024,
